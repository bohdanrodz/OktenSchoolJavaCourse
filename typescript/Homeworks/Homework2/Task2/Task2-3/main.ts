...
const prev:HTMLButtonElement | null = buttons.item(0);
const next:HTMLButtonElement | null = buttons.item(1);
interface Person{
    name: string;
    surname: string;
}
...
const div:HTMLElement | null = document.getElementById('object-wrapper');
let j:number = 0
window.onload = ():void => {
    if (div) {
        for (let i = 0; i < 10; i++) {
            const object:HTMLParagraphElement = document.createElement('p');
